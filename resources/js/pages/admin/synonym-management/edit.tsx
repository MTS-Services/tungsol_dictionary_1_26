...
export default function Create({ WordDefinitions, Words, Synonym }: Props) {
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      router.get(
        route("admin.sm.synonyms.edit", Synonym.id),
        {
  ...
  }, [definitionSearch, wordSearch, Synonym.id]);

  const { data, setData, post, processing, errors } = useForm({
    definition_id: Synonym.definition_id,
    synonym_word_id: Synonym.synonym_word_id,
    relevance_score: Synonym.relevance_score?.toString() ?? "",
    _method: "put",
  });
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route("admin.sm.synonyms.update", Synonym.id), {
      onSuccess: () => {
        toast.success("Synonym updated successfully!");
      },
      onError: () => {
        toast.error("Failed to update synonym. Please check the form for errors.");
      },
    });
  };
  return (
    <AdminLayout activeSlug="synonyms">
      <Head title="Edit Synonym" />

      <div className="flex justify-between mb-6">
        <h1 className="text-2xl font-bold">Edit Synonym</h1>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      value={String(data.synonym_word_id)}
