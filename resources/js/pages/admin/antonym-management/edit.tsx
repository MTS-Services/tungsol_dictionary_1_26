...
export default function Edit({ WordDefinitions, Words, Antonym }: Props) {
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      router.get(
        route("admin.am.antonyms.edit", Antonym.id),
        {
  ...
  }, [definitionSearch, wordSearch, Antonym.id]);

  const { data, setData, post, processing, errors } = useForm({
    definition_id: Antonym.definition_id,
    antonym_word_id: Antonym.antonym_word_id,
    relevance_score: Antonym.relevance_score?.toString() ?? "",
    _method: "put",
  });
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route("admin.am.antonyms.update", Antonym.id), {
      onSuccess: () => {
        toast.success("Antonym updated successfully!");
      },
      onError: () => {
        toast.error("Failed to update antonym. Please check the form for errors.");
      },
    });
  };
  return (
    <AdminLayout activeSlug="antonyms">
      <Head title="Edit Antonym" />

      <div className="flex justify-between mb-6">
        <h1 className="text-2xl font-bold">Edit Antonym</h1>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid gap-2">
                    ...
                    <Select
                      value={String(data.definition_id)}
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      value={String(data.antonym_word_id)}
                      onValueChange={(value) => setData("antonym_word_id", Number(value))}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select antonym word" />
                      </SelectTrigger>
                      <SelectContent>
                        <div className="p-2">
                          <Input
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  >
                    ...
                    {processing ? "Updating..." : "Update Antonym"}
                  </Button>
                </CardContent>
              </Card>
            </div>
